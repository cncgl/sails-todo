...
gulp.task('build.dev', function (done) {
    return utils_1.runSequence('clean.dist', 'tslint', 'build.sass.dev', 'build.assets.dev', 'build.js.dev', 'build.index', done);
});
gulp.task('build.prod', function (done) {
    return utils_1.runSequence('clean.dist', 'clean.tmp', 'tslint', 'build.sass.prod', 'build.assets.prod', 'build.html_css.prod', 'build.deps', 'build.js.prod', 'build.bundles', 'build.index', done);
});
...
gulp.task('test', function (done) {
    return utils_1.runSequence('clean.test', 'tslint', 'build.sass.test', 'build.assets.test', 'build.js.test', 'build.index', 'karma.start', done);
});
